  ...
  <a href="{{ route(Auth::user()->role === 'admin' ? 'admin.clothing-item.show' : 'clothing-item.show', ['clothing_item' => $item->id]) }}"
    ...
    <div class="p-3 text-sm text-gray-800 space-y-1">
        <div class="flex justify-between items-center">
            <div class="text-sm lg:text-base text-gray-600">{{ $item->brand->name }} / {{ $item->category->name ? __("category.{$item->category->name}") : '' }}</div>
            @if($item->status !== 'discarded')
            <form method="post"
                action="{{ route(Auth::user()->role === 'admin' ? 'admin.clothing-item.switchStatus' : 'clothing-item.switchStatus', ['clothing_item' => $item->id]) }}" class="inline-block">
                @csrf
                @method('put')
